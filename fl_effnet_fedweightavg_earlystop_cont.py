...
PATIENCE = 5
MODEL_NAME = f'ff_test_fl_effnet_fedweightedavg_earlystop'
...
if __name__ == '__main__':
    ...
    #checkpoint = torch.load('ff_test_fl_effnet_fedweightedavg_earlystop.pth') #THIS IS GOOD
    checkpoint = torch.load('ff_test_fl_effnet_fedweightedavg_earlystop_earlystop.pth')
    global_model.load_state_dict(checkpoint['model_state'])
    global_model = global_model.to(device)

    COMM_ROUNDS = checkpoint['epoch']
    ...
    for comm_round in range(COMM_ROUNDS+1, comms_round):  
        ...
        global_loss, global_acc = test_model(global_model, val_loader, loss_fn)
