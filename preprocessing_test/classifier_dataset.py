...
class DeepFakeClassifierDataset(Dataset):
        ...
        while True:
            ...
            try:
                ...
                try:
                    ...
                    pass
                if self.mode == "train" and self.hardcore and not self.rotation: # this will run for train
                    ...
                    if os.path.exists(landmark_path) and random.random() < 0.7:
                        ...
                        while current_try < tries:
                            ...
                            if label < 0.5 or np.count_nonzero(mask * bitmap_msk) > 20: # if original, apply mask?
                                mask *= bitmap_msk
                                image *= np.expand_dims(bitmap_msk, axis=-1)
                                break
                            current_try += 1
                ...
                valid_label = np.count_nonzero(mask[mask > 20]) > 32 or label < 0.5 # final check
                valid_label = 1 if valid_label else 0
                rotation = 0
                if self.transforms: # none by default
                    data = self.transforms(image=image, mask=mask)
                    image = data["image"]
                    mask = data["mask"]
                if self.mode == "train" and self.hardcore and self.rotation: # default not this (rotation)
                    # landmark_path = os.path.join(self.data_root, "landmarks", ori_video, img_file[:-4] + ".npy")
                ...
                if self.mode == "train" and self.rotation: # default not this
                    rotation = random.randint(0, 3)
                    image = rot90(image, rotation)

                # at the end, return transformed image
    ...
    def _prepare_data(self, epoch, seed):
        df = self.df
         # i think fold not used anymore
        rows = df[df["fold"] == self.fold]
        seed = (epoch + 1) * seed
        if self.oversample_real: # this is true
    ...
    def _oversample(self, rows: pd.DataFrame, seed):
        ...
        if self.mode == "train":
            fakes = fakes.sample(n=num_real, replace=False, random_state=seed) # ensure fake does not outnumber real
