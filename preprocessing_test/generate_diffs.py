...
from os_helper import get_original_with_fakes
...
def save_diffs(pair, root_dir):
    ...
    for frame in range(320):
        if frame % 10 != 0:
            continue
        for actor in range(2): # this doesnt need i think
            ...
            if os.path.exists(ori_path) and os.path.exists(fake_path):
                ...
                d, a = compare_ssim(img1, img2, channel_axis=-1, full=True) # channel axis -1 means vary along this channel
                a = 1 - a
...
root_dir = os.getcwd()
def main():
    pairs = get_original_with_fakes(root_dir) # get pairs of real and fake images from metadata
    ...
    with Pool(processes=os.cpu_count() - 2) as p:
        with tqdm(total=len(pairs)) as pbar:
            func = partial(save_diffs, root_dir=root_dir) # get diffs between real and fake and save
