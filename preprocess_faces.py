...
def save_face_crops_image(faces, video_save_folder):
    for i in range(len(faces)):
        face = faces[i]
        #face = Image.fromarray(face.numpy())
        #print(face.shape)
...
def preprocess_image(source_folder, to_folder, video_name):
    ...
    try:
        ...
        bbox_path = os.path.join(SOURCE_FOLDER, 'boxes', source_folder,  video_name[:-4] + '.json')
...
DATA_FOLDER = r'D:\dfdc\dfdc_videos'
...
if __name__ == '__main__':
    ...
    out_dir = os.path.join(SOURCE_FOLDER, 'crops', source_folder)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=8) as executor:
        list(tqdm(executor.map(partial(preprocess_image, source_folder, out_dir), video_names)))
