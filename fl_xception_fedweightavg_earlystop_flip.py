...
if __name__ == '__main__':
    ...
    global_model = timm.create_model('xception', pretrained=True, num_classes=1)
    for param in global_model.parameters():
        param.requires_grad = False
    # Unfreeze the final fully connected layer
    for param in global_model.fc.parameters():
    ...
    for comm_round in range(comms_round):  
        ...
        for client in client_names:
            print(client)
            local_model = timm.create_model('xception', pretrained=True, num_classes=1)
            for param in local_model.parameters():
                param.requires_grad = False
            # Unfreeze the final fully connected layer
            for param in local_model.fc.parameters():
                param.requires_grad = True
