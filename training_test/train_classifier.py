...
import sys
sys.path.append(r"C:\Users\Sheryl\Documents\GitHub\dfdc_deepfake_challenge")
from sklearn.metrics import log_loss
from torch import topk

from training_test import losses
from training_test.datasets.classifier_dataset import DeepFakeClassifierDataset
from training_test.losses import WeightedLosses
from training_test.tools.config import load_config
from training_test.tools.utils import create_optimizer, AverageMeter
from training_test.transforms.albu import IsotropicResize
from training_test.zoo import classifiers
...
logdir = 'logs'
zero_score = False
from_zero = False # start training from scratch
distributed = False
freeze_epochs = 0
local_rank = 0 # rank of current process
seed = 777
padding_part = 3
opt_level = 'O1' # optimisation level for mixed precision training
test_every = 1 # test every 1 epoch
no_oversample = False
no_hardcore = True
only_changed_frames = True
output_dir = 'weights/'
def main(): 
    os.makedirs(output_dir, exist_ok=True)
    if distributed:
        torch.cuda.set_device(local_rank)
        torch.distributed.init_process_group(backend='nccl', init_method='env://') # allows to be specified via env
    else:
    ...
    ohem = conf.get("ohem_samples", None) #  Online Hard Example Mining (OHEM), not in config
    reduction = "mean"
    if ohem:
        reduction = "none"
    loss_fn = []
    ...
    data_train = DeepFakeClassifierDataset(mode="train",
                                           oversample_real=not no_oversample,
                                           fold=fold,
                                           padding_part=padding_part,
                                           hardcore=not no_hardcore,
                                           crops_dir=crops_dir,
                                           data_path=data_dir,
    ...
    data_val = DeepFakeClassifierDataset(mode="val",
                                         ...
                                         normalize=conf.get("normalize", None))
    val_data_loader = DataLoader(data_val, batch_size=batch_size * 2, num_workers=workers, shuffle=False,
                                 pin_memory=True) # False
    os.makedirs(logdir, exist_ok=True)
    summary_writer = SummaryWriter(logdir + '/' + conf.get("prefix", prefix) + conf['encoder'] + "_" + str(fold))
    if resume: # specific checkpoint file
        if os.path.isfile(resume):
            print("=> loading checkpoint '{}'".format(resume))
            checkpoint = torch.load(resume, map_location='cpu')
            state_dict = checkpoint['state_dict'] # model weights
            state_dict = {k[7:]: w for k, w in state_dict.items()} # removes module. prefix
            model.load_state_dict(state_dict, strict=False) # load even if some keys dont match
            if not from_zero:
                start_epoch = checkpoint['epoch']
                if not zero_score:
                    bce_best = checkpoint.get('bce_best', 0)
            print("=> loaded checkpoint '{}' (epoch {}, bce_best {})"
                  .format(resume, checkpoint['epoch'], checkpoint['bce_best']))
        else:
    ...
    if conf['fp16']: # is mixed precision training enabled
        pass
        #model, optimizer = amp.initialize(model, optimizer,
        #                                  opt_level=.opt_level,
        #                                  loss_scale='dynamic')

    snapshot_name = "{}{}_{}_{}".format(conf.get("prefix", prefix), conf['network'], conf['encoder'], fold)
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        train_data_loader = DataLoader(data_train, batch_size=batch_size, num_workers=workers,
        ...
        train_epoch(current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, summary_writer, conf,
                    local_rank, only_changed_frames)
        model = model.eval()
        ...
        if local_rank == 0:
            ...
            torch.save({
                ...
                'state_dict': model.state_dict(),
                'bce_best': bce_best,
            }, output_dir + snapshot_name + "_{}".format(current_epoch))
            if (epoch + 1) % test_every == 0:
                bce_best = evaluate_val(val_data_loader, bce_best, model,
...
def evaluate_val(data_val, bce_best, model, snapshot_name, current_epoch, summary_writer):
    print("Test phase")
    model = model.eval()
        ...
        if bce < bce_best:
            ...
            if output_dir is not None:
                torch.save({
                    ...
                    'state_dict': model.state_dict(),
                    'bce_best': bce,
                }, output_dir + snapshot_name + "_best_dice")
            bce_best = bce
            with open("predictions_{}.json".format(fold), "w") as f:
